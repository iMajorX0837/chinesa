 ...
 *  ╚██████╗███████╗██║ ╚███║   ██║   ██║  ██║██║  ██║███████╗
 *   ╚═════╝╚══════╝╚═╝  ╚══╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝
 * DER VALOR A QUEM AJUDOU VAZANDO O CODIGO COLA NA COMUNIDADE https://t.me/centralig
 *  ██╗ ██████╗  █████╗ ███╗  ███╗██╗███╗  ██╗ ██████╗
 *  ██║██╔════╝ ██╔══██╗████╗████║██║████╗ ██║██╔════╝
 *  ██║██║  ███╗███████║██╔████╔██║██║██╔██╗██║██║  ███╗
 *  ██║██║   ██║██╔══██║██║╚██╔╝██║██║██║╚████║██║   ██║
...
ob_start();
error_log("=== API.PHP INICIADA ===", 3, __DIR__ . '/error.log');
...
function sendApiError($code, $message)
{
    ...
    if (is_string($requestPath) && strpos($requestPath, '/trpc/') !== false) {
        $map = [
            400 => ['BAD_REQUEST', -32600],
            401 => ['UNAUTHORIZED', -32001],
            403 => ['FORBIDDEN', -32003],
            404 => ['NOT_FOUND', -32004],
            500 => ['INTERNAL_SERVER_ERROR', -32603],
        ];
        $mapped = $map[$code] ?? ['INTERNAL_SERVER_ERROR', -32603];
...
header('Access-Control-Allow-Headers: Content-Type, Authorization, x-trpc-source');
header('Access-Control-Max-Age: 86400');
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'OPTIONS') {
    http_response_code(204);
    exit;
}
$onlinePingPath = is_string($path) ? rtrim($path, '/') : '';
if ($onlinePingPath === '/api/v1/online_ping' || $onlinePingPath === '/api/v1/online_ping.php' || preg_match('#/online_ping(\.php)?$#', (string)$onlinePingPath)) {
    $rotaEncontrada = true;
...
function fetchGameLogoBytes($bannerUrl) {
    global $WG_BUCKET_SITE;
    $localRoot = dirname(__DIR__, 2);
    $hostBase = rtrim((string)$WG_BUCKET_SITE, '/');
    if ($hostBase !== '' && strpos($bannerUrl, $hostBase) === 0) {
        $relative = parse_url(substr($bannerUrl, strlen($hostBase)), PHP_URL_PATH);
        $localPath = $localRoot . str_replace('/', DIRECTORY_SEPARATOR, $relative);
        if (is_file($localPath)) {
            return file_get_contents($localPath);
        }
    }
    if (strpos($bannerUrl, '/') === 0 && strpos($bannerUrl, '//') !== 0) {
        $localPath = $localRoot . str_replace('/', DIRECTORY_SEPARATOR, $bannerUrl);
        if (is_file($localPath)) {
            return file_get_contents($localPath);
        }
    }
    ...
    curl_close($ch);
    if ($bytes === false || $status >= 400 || $bytes === '') {
        return false;
    }
    return $bytes;
}
if (strpos($path, '/api/frontend/game-logo/') === 0) {
    ...
    $file = urldecode($file);
    $name = preg_replace('/\\.(jpg|jpeg|png|webp|jfif)$/i', '', $file);
    $prov = '';
    $gameKey = '';
    $pos = strpos($name, '_');
    if ($pos !== false) {
    ...
    if ($bannerUrl) {
        $cacheDir = dirname(__DIR__, 2) . '/uploads/game-logos';
        $cacheKey = preg_replace('/[^a-zA-Z0-9._-]/', '', $name);
        if ($cacheKey === '') $cacheKey = md5($bannerUrl);
        $cacheFile = $cacheDir . '/' . $cacheKey;
        $bytes = null;
        if (is_file($cacheFile) && filesize($cacheFile) > 32) {
            $bytes = file_get_contents($cacheFile);
        } else {
            $bytes = fetchGameLogoBytes($bannerUrl);
        ...
        if ($bytes !== false && $bytes !== null && strlen($bytes) > 32) {
            $mime = guessImageMimeFromBytes($bytes, $bannerUrl);
    ...
    $fallbackUrl = 'https://77qw7.com' . $path;
    $bytes = fetchGameLogoBytes($fallbackUrl);
    if ($bytes !== false && strlen($bytes) > 32) {
        $cacheDir = dirname(__DIR__, 2) . '/uploads/game-logos';
        $cacheKey = preg_replace('/[^a-zA-Z0-9._-]/', '', $name);
        if ($cacheKey === '') $cacheKey = md5($fallbackUrl);
        if (!is_dir($cacheDir)) {
            @mkdir($cacheDir, 0755, true);
        }
        @file_put_contents($cacheDir . '/' . $cacheKey, $bytes);
        $mime = guessImageMimeFromBytes($bytes, $fallbackUrl);
        header_remove('Content-Type');
        header('Content-Type: ' . $mime);
        header('Cache-Control: public, max-age=86400');
        echo $bytes;
...
function getRequestToken() {
    ...
    if ($token === '' && !empty($_COOKIE['token_user'])) {
        $token = (string)$_COOKIE['token_user'];
    }
...
function setAuthCookie($token, $ttlSeconds = 604800) {
    $expire = time() + (int)$ttlSeconds;
    $secure = isHttpsRequest() ? '; Secure' : '';
    $expiresGmt = gmdate('D, d-M-Y H:i:s T', $expire);
    header(
        'Set-Cookie: token_user=' . rawurlencode((string)$token)
        . '; Expires=' . $expiresGmt
        . '; Max-Age=' . (int)$ttlSeconds
        . '; Path=/'
        . '; HttpOnly'
        . '; SameSite=Lax'
        . $secure,
...
function clearAuthCookie() {
    ...
    foreach (['token_user', 'token'] as $name) {
        header(
            'Set-Cookie: ' . $name . '=; Expires=' . $past . '; Max-Age=0; Path=/; HttpOnly; SameSite=Lax' . $secure,
            false
        );
    }
}
...
if ($path === '/api/frontend/trpc/auth.registe') {
    ...
    if (strlen($password) < 6) {
        sendApiError(400, "Senha deve ter no mínimo 6 caracteres");
    }
    ...
    $stmt->bind_param("isssssssssii", $userId, $mobile, $mobile, $passwordHash, $senhaparasacar_empty, $url, $token, $data_registro, $invite_code, $invitation_code_final, $tipo_pagamento, $statusaff);
    if ($stmt->execute()) {
        if (!empty($inviteCode)) {
...
if ($path === '/api/frontend/trpc/registerReward.receive') {
    ...
    try {
        ...
        $mysqli->rollback();
        error_log("registerReward.receive failed uid={$user['id']}: " . $e->getMessage());
        sendTrpcResponse(["status" => false, "message" => "Internal error"]);
    }
...
if ($path === '/api/frontend/trpc/auth.login') {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        $pwInfo = password_get_info($storedPw);
        $pwOk = false;
        if (!empty($pwInfo['algo'])) {
            $pwOk = password_verify($password, $storedPw);
        } else if (hash_equals((string)$storedPw, (string)$password)) {
            $pwOk = true;
            $rehash = password_hash($password, PASSWORD_BCRYPT);
            $stmtUp = $mysqli->prepare("UPDATE usuarios SET password = ? WHERE id = ?");
            $stmtUp->bind_param("si", $rehash, $row['id']);
            $stmtUp->execute();
            $stmtUp->close();
        }
        if ($pwOk) {
            ...
            $userId = $row['id'];
            $stmtTok = $mysqli->prepare("UPDATE usuarios SET token = ? WHERE id = ?");
            $stmtTok->bind_param("si", $token, $userId);
            $stmtTok->execute();
            $stmtTok->close();
            $isHttps = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') || (($_SERVER['HTTP_X_FORWARDED_PROTO'] ?? '') === 'https');
            setcookie('token_user', $token, [
            ...
            sendApiError(401, "Credenciais inválidas");
        }
    } else {
        sendApiError(401, "Credenciais inválidas");
    }
...
if ($path === '/api/frontend/trpc/auth.logout') {
    ...
    $user = getCurrentUser($mysqli);
    if ($user) {
        $stmtInv = $mysqli->prepare("UPDATE usuarios SET token = NULL WHERE id = ?");
        $stmtInv->bind_param("i", $user['id']);
        $stmtInv->execute();
        $stmtInv->close();
    }
    $isHttps = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') || (($_SERVER['HTTP_X_FORWARDED_PROTO'] ?? '') === 'https');
    $clearOpts = ['expires' => time() - 3600, 'path' => '/', 'httponly' => true, 'secure' => $isHttps, 'samesite' => 'Lax'];
    setcookie('token_user', '', $clearOpts);
    setcookie('token', '', $clearOpts);
    if (session_status() === PHP_SESSION_ACTIVE) {
        session_destroy();
    }
    sendTrpcResponse(new stdClass());
...
if (strpos($path, '/reward.receive') !== false) {
    ...
    $logFile = __DIR__ . '/reward_debug.log';
    $rawInput = file_get_contents('php://input');
...
if ($path === '/api/frontend/trpc/withdraw.createOrder') {
    ...
    $storedAssetPw = (string)($user['senhaparasacar'] ?? '');
    $assetOk = false;
    if ($storedAssetPw === '') {
        $assetOk = false;
    } else {
        $assetInfo = password_get_info($storedAssetPw);
        if (!empty($assetInfo['algo'])) {
            $assetOk = password_verify((string)$password, $storedAssetPw);
        } else if (hash_equals($storedAssetPw, (string)$password)) {
            $assetOk = true;
            $rehashA = password_hash((string)$password, PASSWORD_BCRYPT);
            $stmtAp = $mysqli->prepare("UPDATE usuarios SET senhaparasacar = ? WHERE id = ?");
            $stmtAp->bind_param("si", $rehashA, $user['id']);
            $stmtAp->execute();
            $stmtAp->close();
        }
    }
    ...
    $transacaoId = bin2hex(random_bytes(12));
    $mysqli->begin_transaction();
    try {
        $stmtDeb = $mysqli->prepare("UPDATE usuarios SET saldo = saldo - ? WHERE id = ? AND saldo >= ?");
        $stmtDeb->bind_param("did", $amount, $user['id'], $amount);
        $stmtDeb->execute();
        if ($stmtDeb->affected_rows !== 1) {
            $stmtDeb->close();
            $mysqli->rollback();
            header('Content-Type: application/json');
            echo json_encode([
        ...
        try { $mysqli->rollback(); } catch (Throwable $ign) {}
        error_log("withdraw.createOrder failed uid={$user['id']}: " . $e->getMessage());
        header('Content-Type: application/json');
        echo json_encode([
            "error" => [
                "json" => [
        ...
        if ($gameId) {
            ...
            if ($resGame->num_rows > 0) {
                ...
                if (isset($gameLaunch['gameURL']) && !empty($gameLaunch['gameURL'])) {
                    ...
                    $launchError = $gameLaunch['error'] ?? 'Erro desconhecido';
                    if (stripos($launchError, 'ERROR_GET_BALANCE_END_POINT') !== false) {
                        $callbackUrl = rtrim(url_sistema(), '/') . '/igamewin';
                        $launchError = "A iGameWin não conseguiu consultar o saldo. No painel da iGameWin, configure o Endpoint/Callback para {$callbackUrl}. Essa URL precisa ser pública na internet; localhost não funciona.";
                    }
                    sendApiError(500, $launchError);
                }
            } else {
