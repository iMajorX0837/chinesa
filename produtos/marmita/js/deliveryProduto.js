...
function totalProduto() {
	...
	let precoHidden = $("#detalhesProduto .info1 .descricao > .preco > span[style*='display:none']").text().trim();
	let precoProduto = parseFloat(precoHidden) || parseFloat($("#detalhesProduto .info3 .precoProduto").text()) || 0;
	if ((!precoProduto || isNaN(precoProduto)) && typeof obterPrecoProduto === 'function') {
		let nomeProduto = $("#detalhesProduto .info1 h3").text().trim();
		precoProduto = obterPrecoProduto(nomeProduto, 0);
	}
	let qtdeProduto = $("#detalhesProduto .info3 .qtdeProduto .qtde").val();
	let totalProduto = 0;
	if (totalOpcionais == 0) {
