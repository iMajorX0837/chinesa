 ...
 */
function normalizeIgamewinProvider($provider)
{
    $key = strtoupper(trim((string)$provider));
    $map = [
        'PG' => 'PGSOFT',
        'PGSOFT' => 'PGSOFT',
        'KKGAME' => 'PGSOFT',
        ...
        'PLAYSON' => 'slot-playson',
        'HABANERO' => 'slot-habanero',
        'HACKSAW' => 'slot-hacksaw',
        'EVOPLAY' => 'slot-evoplay',
        'JILI' => 'slot-jili',
        'CP' => 'cpgames',
        'G759' => 'original',
    ];
    return $map[$key] ?? trim((string)$provider);
}
...
function pegarLinkJogoigamewin($provider, $game, $email)
{
    ...
    $homeUrl = url_sistema();
    $providerCode = normalizeIgamewinProvider($provider);
    ...
    $createResp = enviarRequest($url, $createPayload);
    logIgamewin("user_create: $createResp");
