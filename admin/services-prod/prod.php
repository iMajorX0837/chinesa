...
function next_sistemas_qrcode($valor, $nome, $id, $comissao = null, $afiliado_id = null, $payTypeSubListId = null, $joinBonus = true)
{
    ...
    if ($greepay_ativo == 1) { $tentativas[] = 'greepay'; }
...
function greepayAuth()
{
    ...
    return [
        'client_id' => $data_greepay['client_id'],
        'client_secret' => $data_greepay['client_secret']
    ];
}
...
function criarQrGreePay($valor, $nome, $id, $comissao = null, $afiliado_id = null, $payTypeSubListId = null, $joinBonus = true)
{
    ...
    $auth = greepayAuth();
    $base = rtrim($data_greepay['url'] ?? 'https://api.codexpay.app', '/');
    $arraypix = [
        "057.033.734-84", "078.557.864-14", "094.977.774-93",
        "033.734.824-37", "091.665.934-84", "081.299.854-54",
        "086.861.364-94", "033.727.064-39"
    ...
    $depositPayload = [
        "amount" => (float)$valor,
        "external_id" => $external_id,
        "clientCallbackUrl" => $url_base . 'callbackpayment/greepay.php',
        "payer" => [
            "name" => $nome ?: "Cliente",
            "email" => $email,
            "document" => preg_replace('/[^0-9]/', '', $cpf)
